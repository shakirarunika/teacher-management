                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-12">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
                                ...
                                <div className="flex gap-2">
                                    <motion.button
                                        ...
                                        className="bg-indigo-50 hover:bg-indigo-100 disabled:opacity-40 text-indigo-700 px-4 py-3 rounded-xl font-bold text-sm transition-colors flex items-center gap-2 shadow-sm border border-indigo-100"
                                    >
                                    ...
                                    <motion.button
                                        ...
                                        className="bg-gray-900 hover:bg-indigo-600 disabled:opacity-40 text-white px-4 py-3 rounded-xl font-bold text-sm transition-colors flex items-center gap-2 shadow-lg"
                                    >
                        ...
                        >
                            {[
                                ...
                                <motion.div
                                    ...
                                    className={`bg-gradient-to-br ${stat.color} rounded-2xl p-5 text-white shadow-lg ${stat.shadow}`}
                                >
                    ...
                    {summary.length > 0 && (
                        <motion.div
                            ...
                            className="bg-white/70 backdrop-blur-xl border border-white rounded-[2rem] p-6 shadow-xl shadow-indigo-100/50 space-y-4"
                        >
                            {/* Rate bar */}
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-sm font-bold text-gray-600">Tingkat Kehadiran Kelas</span>
                                ...
                                <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {atRiskStudents.map(s => (
                                            <span key={s.id} className="inline-flex items-center gap-1.5 bg-rose-100 text-rose-700 text-xs font-bold px-3 py-1.5 rounded-full">
                                                {s.name}
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'summary' ? (
                            ...
                            <motion.div
                                ...
                                className="bg-white/70 backdrop-blur-xl border border-white rounded-[2rem] shadow-xl overflow-hidden"
                            >
                                {summary.length === 0 ? (
                                    <div className="py-16 text-center">
                                        <CalendarDaysIcon className="w-12 h-12 text-gray-200 mx-auto mb-4" />
                                        <h3 className="text-lg font-bold text-gray-400">Belum Ada Data Absensi</h3>
                                        <p className="text-gray-400 text-sm mt-1">Tidak ada absensi yang tercatat untuk {monthLabel} {filters.year}.</p>
                                    </div>
                                ) : (
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-gray-50/70 border-b border-gray-100">
                                                    <th className="py-4 px-6 font-black text-gray-400 text-xs uppercase tracking-wider">No</th>
                                                    <th className="py-4 px-6 font-black text-gray-500 text-xs uppercase tracking-wider">Nama Siswa</th>
                                                    <th className="py-4 px-6 font-black text-emerald-600 text-xs uppercase tracking-wider text-center">Hadir</th>
                                                    <th className="py-4 px-6 font-black text-amber-500 text-xs uppercase tracking-wider text-center">Sakit</th>
                                                    <th className="py-4 px-6 font-black text-blue-500 text-xs uppercase tracking-wider text-center">Izin</th>
                                                    <th className="py-4 px-6 font-black text-rose-500 text-xs uppercase tracking-wider text-center">Alpha</th>
                                                    <th className="py-4 px-6 font-black text-gray-400 text-xs uppercase tracking-wider text-center">% Hadir</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {summary.map((row, idx) => {
                                                    ...
                                                    return (
                                                        <motion.tr
                                                            ...
                                                            initial="hidden"
                                                            animate="show"
                                                            className={`border-b border-gray-50 transition-colors ${isRisk ? 'bg-rose-50/40 hover:bg-rose-50/60' : 'hover:bg-white/60'}`}
                                                        >
                                                            <td className="py-4 px-6 text-gray-400 font-bold text-sm">{idx + 1}</td>
                                                            <td className="py-4 px-6">
                                                                <div className="flex items-center gap-2">
                                                                    <div className="font-bold text-gray-900">{row.name}</div>
                                                                    {isRisk && <ExclamationTriangleIcon className="w-4 h-4 text-rose-500 flex-shrink-0" />}
                                                                </div>
                                                                <div className="text-xs font-medium text-gray-400 mt-0.5">NIS: {row.nis}</div>
                                                            </td>
                                                            <td className="py-4 px-6 text-center font-black text-emerald-600 text-lg">{row.hadir}</td>
                                                            <td className="py-4 px-6 text-center font-black text-amber-500 text-lg">{row.sakit}</td>
                                                            <td className="py-4 px-6 text-center font-black text-blue-500 text-lg">{row.izin}</td>
                                                            <td className={`py-4 px-6 text-center font-black text-lg ${isRisk ? 'text-rose-600' : 'text-rose-400'}`}>{row.alpha}</td>
                                                            <td className="py-4 px-6 text-center">
                                                                <div className="flex flex-col items-center gap-1">
                                                                    <span className={`text-sm font-black ${pct >= 80 ? 'text-emerald-600' : pct >= 60 ? 'text-amber-600' : 'text-rose-600'}`}>
                                                                        {pct}%
                                                                    </span>
                            ...
                            >
                                {recordedDates.length === 0 ? (
                                    <div className="py-16 text-center">
                                        <CalendarDaysIcon className="w-12 h-12 text-gray-200 mx-auto mb-4" />
                                        <h3 className="text-lg font-bold text-gray-400">Belum Ada Data Harian</h3>
                                        <p className="text-gray-400 text-sm mt-1">Tidak ada absensi yang tercatat untuk {monthLabel} {filters.year}.</p>
                                    </div>
                                ) : (
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left border-collapse min-w-max">
                                            <thead>
                                                <tr className="bg-gray-50/70 border-b border-gray-100">
                                                    <th className="py-4 px-4 font-black text-gray-500 text-xs uppercase tracking-wider sticky left-0 bg-gray-50/90 backdrop-blur-sm z-10 min-w-[180px]">
                                                        Nama Siswa
                                                    </th>
                                                    {recordedDates.map(date => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                <div className={`text-[10px] font-semibold capitalize ${holiday ? 'opacity-95' : 'text-gray-300'}`}>
                                            ...
                                            <tbody>
                                                {detail.map((student, idx) => (
                                                    <motion.tr
                                                        ...
                                                        initial="hidden"
                                                        animate="show"
                                                        className="border-b border-gray-50 hover:bg-white/60 transition-colors"
                                                    >
                                                        <td className="py-3 px-4 sticky left-0 bg-white/80 backdrop-blur-sm z-10">
                                                            <div className="flex items-center gap-2">
                                                                <div className="w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs font-black flex-shrink-0">
                                                                    {idx + 1}
                                                                </div>
                                                                <div>
                                                                    <div className="font-bold text-gray-900 text-sm leading-tight">{student.name}</div>
                                                        ...
                                                        {recordedDates.map(date => {
                                                            ...
                                                            let displayChar = '·';
                                                            let customStyle = 'text-gray-300';

                                                            if (status !== '-') {
                                                                const abbrev = { 'Hadir': 'H', 'Sakit': 'S', 'Izin': 'I', 'Alpha': 'A' };
                                                                displayChar = abbrev[status] ?? '·';
                                                                customStyle = STATUS_STYLE[status] ?? 'text-gray-300';
                                                            } else if (holiday) {
                                                                displayChar = 'L';
                                                                customStyle = holiday.type === 'Nasional'
                                                                    ? 'bg-rose-100 text-rose-700 font-bold border border-rose-200 shadow-sm'
                                                                    : 'bg-amber-100 text-amber-700 font-bold border border-amber-200 shadow-sm';
                                                            }

                                                            return (
                                                                <td
                                                                    key={date}
                                                                    className={`py-3 px-2 text-center ${holiday ? 'bg-gray-50/20' : ''}`}
                                                                    title={holiday ? `${holiday.name} (${holiday.type})` : undefined}
                                                                >
                                        ...
                                        {/* Legend */}
                                        <div className="flex flex-wrap items-center gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/30">
                                            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Keterangan:</span>
                                            {[
                                                { label: 'H — Hadir',  cls: 'bg-emerald-100 text-emerald-700' },
                                                { label: 'S — Sakit',  cls: 'bg-amber-100 text-amber-700' },
                                                { label: 'I — Izin',   cls: 'bg-blue-100 text-blue-700' },
                                                { label: 'A — Alpha',  cls: 'bg-rose-100 text-rose-700' },
                                                { label: '· — Tidak dicatat', cls: 'text-gray-300' },
                                            ].map(l => (
