                                    ...
                                    <div key={i}>
                                        ...
                                        {stat.type === 'isian' && (
                                            <div className="text-xs space-y-1">
                                                <p className="text-emerald-600 dark:text-emerald-400 font-bold">Kunci: <MathText text={stat.answer} /></p>
                                                {Object.keys(stat.wrong).length > 0 && (
                                                    <p className="text-gray-500 dark:text-slate-400">
                                                        Jawaban salah terbanyak: {Object.entries(stat.wrong).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([t, c]) => `"${t}" (${c}×)`).join(', ')}
                                                    </p>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {stat.type === 'jodoh' && (
                                            <p className="text-xs text-gray-500 dark:text-slate-400">
                                                {stat.pairsTotal} pasangan · rata-rata {Math.round(stat.matchedSum / (attempts.length * stat.pairsTotal) * 100)}% pasangan terjawab benar
                                            </p>
                                        )}
